...
- 生成日：初版 2026-07-01／**改善版へ差し替え 2026-08-17**（生成AI感の強いタッチを改善した新版2枚。いずれもフリープラン）
- 生成者：umada kenshun（プロンプト指示による生成）
- ファイル：口開け `mascot_open.png` ／ 口閉じ `mascot_closed.png` の2点（差し替え後もファイル名・2枚構成は不変）
...
- 類似性確認：既存の有名キャラクターに似ていないことを目視確認（初版 2026-07-26／改善版 2026-08-17、いずれも生成者本人。Assets 取り込み時に実施）
...
| 2026-08-17 | マスコット2枚を改善版に差し替え（Phase 4-8）：生成AI感の強いタッチを改善した新版（同日 ChatGPT 生成）。ライセンス根拠は従来と同一（OpenAI 規約）、類似性確認も改善版で再実施 |
