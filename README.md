...
- 🔶 **Phase 3：コア機能開発**（進行中）— 3-1〜3-10 完了（ホーム画面と画面切替メカニズム、起動時免責モーダル、単語登録画面、フォトライブラリ連携、写真なし単語のフォールバック表示・一覧サムネイル、単語削除と下限20語維持、プリセット復元、問題画面 UI、ゲームロジック〔出題生成・進行管理〕、音声読み上げ〔出題プロンプトの自動再生〕、読み上げキャラクター〔口パク演出〕、正解演出〔タップ回答・効果音・自動遷移〕）、以降は 不正解演出 / 結果画面を順次実装
...
│   │   ├── GameView.swift       # 問題画面（出題表示・タップ回答・正解演出、GameSession を @State 所有）
